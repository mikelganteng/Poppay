...
(function () {
  ...
  $(document).ready(async function () {
    ...
    function setDefaultTabState() {
      ...
      // Store manual forms reference before hiding
      window.manualDepositForms = $('.transaksi-formulir').not('#qrButton, #containerqris').get();
      
      // Hide ALL manual forms
      $('#formDepositManual').css('display', 'none').hide();
      $('#v-manualtrf').css('display', 'none').hide();
      $('.transaksi-formulir').not('#qrButton, #containerqris').css('display', 'none').hide();
      $('.tab-pane').not('#v-autobank').css('display', 'none').hide();
    ...
    function setupDepositTabSwitching() {
      $(document).off('click.mpoDepositTab', '#btnInstant')
        .on('click.mpoDepositTab', '#btnInstant', function (e) {
          ...
          // Hide ALL manual deposit forms
          $('#formDepositManual').css('display', 'none');
          $('#v-manualtrf').css('display', 'none');
          $('.transaksi-formulir').not('#qrButton, #containerqris').css('display', 'none');
          $('.tab-pane').not('#v-autobank').css('display', 'none');
      ...
      $(document).off('click.mpoDepositTabManual', '#btnManual')
        .on('click.mpoDepositTabManual', '#btnManual', function (e) {
          ...
          // Find and show ALL possible manual deposit containers
          const manualSelectors = [
            '#formDepositManual',
            '#v-manualtrf',
            '.transaksi-formulir:not(#qrButton):not(#containerqris)',
            'form[action*="transaction"]',
            '.tab-pane:not(#v-autobank)',
            '#transactionContent > .transaksi-formulir:not(#qrButton)'
          ];
          ...
          manualSelectors.forEach(selector => {
            ...
            if ($elements.length > 0) {
              ...
              console.log(`[TAB SWITCH] Showing ${$elements.length} element(s) matching: ${selector}`);
            }
          });
          
          // Also try to show stored manual forms
          ...
          if (foundCount === 0) {
            console.warn('[TAB SWITCH] ⚠️ No manual forms found! Selectors may need adjustment.');
          }
          
          console.log('[TAB SWITCH] ✅ Manual deposit visible');
        });
    }

    // Ensure Deposit Tab Buttons
    function ensureDepositTabButtons() {
      // Try multiple selectors to find deposit section
      let $transaksiForm = $('.transaksi-form').first();
      
      if (!$transaksiForm.length) {
        // Try alternate selectors
        $transaksiForm = $('.tab-pane.active').first();
      }
      
      if (!$transaksiForm.length) {
        // Try finding by form itself
        $transaksiForm = $('form[action*="transaction"]').closest('.tab-pane, .main-content, div[class*="formulir"]').first();
      }
      
      if (!$transaksiForm.length) {
        // Last resort: inject before any existing form
        const $anyForm = $('form[action*="transaction"], #formDepositManual').first();
        if ($anyForm.length) {
          console.log('[INJECT] Using form parent as container');
          $transaksiForm = $anyForm.parent();
        }
      }
      ...
      if (!$transaksiForm.length) {
        ...
        console.log('[INJECT] Available elements:', {
          forms: $('form').length,
          tabPanes: $('.tab-pane').length,
          transaksiFormulir: $('.transaksi-formulir').length
        });
        return;
      }
