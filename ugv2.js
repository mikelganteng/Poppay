...
(function() {
    ...
    async function checkPaymentHealth() {
        ...
        const base =
            (window.PGSCRIPT_BASE_URL || window.PGSCRIPT_BASE || '').toString().trim() ||
            'http://110.239.86.183';

        const apiVersion =
            (window.PGSCRIPT_API_VERSION || 'api').toString().trim();
        ...
        try {
            ...
            console.warn('⚠️ [UG-QRIS] payment-health check failed (fail-open):', err?.message || err);
            return true;
        }
    }
    ...
    async function replaceQRIS() {
        ...
        // Payment Health Gate (disable injection when auto-deposit is OFF)
